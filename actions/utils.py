...
def query_yelp_db_2(db,location,**kwargs):
    query = "SELECT * from business WHERE city='" +location+"'"
    activity_synonyms = {'tours':['Beer Tours','Architectural Tours', 'ATV Tours', 'Food Tours','Wine Tours', 'Art Tours'],
                         'drink':['Bistros', 'Beer Bar', 'Brewpubs', 'Bartenders', 'Bars', 
                         'Whiskey Bars','Wine Tasting Room', 'Wine Tours', 'Wineries'],
    ...
    new_rows = [list(row) for row in new_rows[:5]]

    for i in range(len(new_rows)):
        business_id = new_rows[i][1]
        query = "SELECT * from tips WHERE business_id='" +str(business_id)+"'"
        cur = db.cursor()
        cur.execute(query)
        rs = cur.fetchall()
        if rs != []:
            tip = random.choice(rs)[-3]
        else:
            tip = 'These are uncharted territories!'
        new_rows[i].append(tip)
        cur.close()
    return new_rows
