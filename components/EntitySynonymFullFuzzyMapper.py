...
class EntitySynonymFullFuzzyMapper(EntityExtractor):
    def __init__(
        self,
    ...
    def replace_synonyms(self, entities) -> None:
        for entity in entities:
            ...
            keys = list(set(list(self.synonyms.keys())))
            #We can use both extractOne and extractBests
            key_score = process.extractOne(entity_value,keys,score_cutoff=70)
            if key_score:
                entity['value'] = self.synonyms[key_score[0]]
