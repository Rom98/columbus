...
class EntitySynonymFuzzyMapper(EntityExtractor):
    def __init__(
        self,
    ...
    def replace_synonyms(self, entities) -> None:
        for entity in entities:
            ...
            values = list(set(list(self.synonyms.values())))
            #print(values)
            for value in values:
                score = fuzz.partial_ratio(entity_value,value)
                scores_list.append([score,value])
            scores_list = sorted(scores_list,reverse=True)
            #print(scores_list)
            best_score = scores_list[0][0]
            if best_score > 70:
                entity['value'] = scores_list[0][1]
